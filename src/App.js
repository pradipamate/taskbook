...
import { Add, RemoveItemFromList, updated,ChangeStatus } from "./practice/actions/TaskbookAction";
import { Col, Button, Container, Row,Form, variant, Modal,Table} from 'react-bootstrap';
...
class App extends Component {
   ...
   Descriptionchangehandler = (event) => {
    ...
    submithandler = (event) => {
       ...
       if (dd < 10) {
        ...
        if(this.state.Title!==null && this.state.Description!==null && this.state.Duedate!==null && this.state.Priority!==null){
            if (this.state !== "") {
                 var data = {};
                ...
                this.setState({ Title: "", Description: "", Duedate: "", Priority: "" });
               }
    ...
    Edithandler = (event) => {
        ...
        const data = AvailableData.find(item => item.id === edit_id);
        this.setState({
            Edittaskmodal: true,
            editid: edit_id,
    ...
    DeleteModalhandler=(id)=> {
      ...
      //  console.log("statedelete1",this.props.Listdata)
        this.setState({
    ...
    DeleteTaskhandler=()=>{
        //debugger;
       //  console.log("statedelet2",this.props.Listdata)
   ...
   //for comlete status //
    ...
    SearchTaskhnadler=(event)=>{
         ...
         if(searchtext!==""){
                    const newData = AvailableData.filter(function(item) {
   ...
   GroupSearchchangehandler=(event)=>{
    ...
    if(selectedvalue=="none"){
        console.log("1");
            let MatcheData=AvailableData.filter(function(obj){
                    for(var key in obj){
            ...
            MyDisplayData:MatcheData
              ...
              var AvailableData=this.props.Listdata;
             let SortCreatedat=AvailableData.sort(function(a,b){
           ...
           let MatchDataPending=AvailableData.filter(function(obj){
                     for(var key in obj){
             ...
             let SortDuedata=MatchDataPending.sort(function(a,b){
         ...
         let MatchDataLow=AvailableData.filter(function(obj){
                     for(var key in obj){
         ...
         let MatchDatamedium=AvailableData.filter(function(obj){
                    for(var key in obj){
        ...
        let MatchDatahigh=AvailableData.filter(function(obj){
                    for(var key in obj){
         ...
         let MatchDatanone=AvailableData.filter(function(obj){
                    for(var key in obj){
       ...
       var  modifieddata=[...MatchDataLow,...MatchDatamedium,...MatchDatahigh,...MatchDatanone];
          this.setState({
   ...
   SortTaskhandler=()=>{
      var AvailableData = this.state.MyDisplayData;
      var SortData=AvailableData.reverse();
      this.setState({MyDisplayData:SortData}) 
   }
  Taskdataview=(id)=>{
    var AvailableData = this.state.MyDisplayData;
    var Requestviewdata=AvailableData.find(item=>item.id===id);
    this.setState({
            Viewtaskmodal: true,
          ...
          var  AllTaskList = AvailableData.map(item => (
                <tr key={item.id} className={item.CurrentState==="Done"?"done":""} >
                  ...
                  <td>
                   ...
                   <i className="fa fa-pencil-square-o" aria-hidden="true" id={item.id} onClick={this.Edithandler}></i>
                    </Button>

                     {item.CurrentState=="pending" ? <Button variant="success mr-2" id={item.id} onClick={this.CompleteTaskhandler}>
                          Done </Button> :<Button variant="secondary mr-2" className="Reopen-button" id={item.id}>
                          Re-open </Button> } 
                          
                      {/* <Button variant="danger">
                      <i className="fa fa-trash" aria-hidden="true" onClick={()=>this.DeleteModalhandler(item.id)}></i>
                      </Button> */}
                  </td>
               </tr>
            ));
            ...
            var  TaskPendingList = DisplayPendingtask.map(item => (
                <tr key={item.id}>
                  <td onClick={() => this.Taskdataview(item.id)}>{item.Title}</td>
                  <td>{item.Priority}</td>
                  <td>{item.CreatedAt}</td>
                  <td>{item.Duedate}</td>
                  <td>
                   <Button variant="primary mr-2 ">
                    <i className="fa fa-pencil-square-o" aria-hidden="true" id={item.id} onClick={this.Edithandler}></i></Button>
                          <span> {item.CurrentState==="pending" ? <Button className="Reopen-button" variant="success mr-2" id={item.id} onClick={this.CompleteTaskhandler}>
                          Done </Button> :<Button variant="secondary mr-2" id={item.id}>
                          Re-open </Button> } 
                          </span> 
                    ...
                    <Row>
                      <Col sm={6} className="text-left">
                       ...
                       <Col sm={6} className="text-right">
                           <div>
                              <i className="fa fa-plus-circle fa-2x" aria-hidden="true" onClick={this.AddInfoHandler}></i>
                          </div>
                      </Col>
                    </Row>
                    <Row>
                       <Col sm={4} className="text-left">
                         <Form.Label> Group By</Form.Label>
                            <Form.Control as="select" onChange={this.GroupSearchchangehandler} name="groupby" >
                       ...
                       <Col sm={8}>
                        <Form.Label>Search</Form.Label>
                           <Form.Control type="text" name="search" placeholder="Search Tasks Name" onChange={this.SearchTaskhnadler} />
                      </Col>
                    </Row>
                   <div>
                    <Modal show={this.state.Addtaskmodal} >
                        ...
                        <Modal.Body>
                            <Form onSubmit={this.submithandler}>
                                <Row>
                                    <Col sm={12}>
                                        <Form.Control type="hidden" name="currentstate" value="pending" />
                                        <Form.Group controlId="formBasicTitle">
                                            <Form.Label>Title</Form.Label>                     
                                            
                                            <Form.Control type="text" onChange={this.Titlechangehandler} name="Title" maxLength="140" minLength="10"   required/>
                                        </Form.Group>
                                    </Col>
                                    <Col sm={12}>
                                        <Form.Group controlId="formBasicDescription">
                                            <Form.Label> Description</Form.Label>
                                            <Form.Control as="textarea" rows="3" onChange={this.Descriptionchangehandler} maxLength="500" minLength="10" name="Description"  required />
                                        </Form.Group>
                                    </Col>
                                    <Col sm={6}>
                                        <Form.Group controlId="formBasicDueDate">
                                            <Form.Label> DueDate</Form.Label>
                                            <Form.Control type="date" onChange={this.Duedatechangehandler} name="Duedate"  required />
                                        </Form.Group>
                                    </Col>
                                    <Col sm={6}>
                                        <Form.Group controlId="Priority">
                                            <Form.Label> Priority</Form.Label>
                                             <Form.Control as="select" onChange={this.Prioritychangehandler} name="Priority" defaultValue={this.state.Priority} required>
                                ...
                                <Row>
                                    <Col sm={12}>
                                        ...
                                        <Form.Group controlId="Summery">
                                            ...
                                            <Form.Control type="text" onChange={this.UpdateTitlechangehandler} name="Title" maxLength="140" minLength="10" value={this.state.updatedTitle} />
                                        </Form.Group>
                                    </Col>
                                    <Col sm={12}>
                                        <Form.Group controlId="Description">
                                            <Form.Label> Description</Form.Label>
                                            <Form.Control as="textarea" rows="3" onChange={this.UpdateDescriptionchangehandler} maxLength="500" minLength="10" name="Description" value={this.state.updatedDescription} />
                                        </Form.Group>
                                    </Col>
                                    <Col sm={6}>
                                        <Form.Group controlId="DueDate">
                                            <Form.Label> DueDate</Form.Label>
                                            <Form.Control type="date" onChange={this.UpdateDuedatechangehandler} name="Duedate" value={this.state.updatedDuedate} />
                                        </Form.Group>
                                    </Col>
                                    <Col sm={6}>
                                        <Form.Group controlId="Priority">
                                            <Form.Label> Priority</Form.Label>
                                             <Form.Control as="select" onChange={this.UpdatePrioritychangehandler} name="Priority" value={this.state.updatedPriority} >
                                                <option selected>Select Value</option>
                                                <option value="none">None</option>
                                                <option value="low">Low</option>
                                                <option value="medium">Medium</option>
                            ...
                            <Form >
                                <Row>
                                    <Col sm={12}>
                                        ...
                                        <Form.Group controlId="Summery">
                                            <Form.Label>Summery</Form.Label>
                                            <Form.Control type="text" onChange={this.UpdateTitlechangehandler} name="Title" value={this.state.viewTitle}  readOnly />
                                        </Form.Group>
                                    </Col>
                                    <Col sm={12}>
                                        <Form.Group controlId="Description">
                                            <Form.Label> Description</Form.Label>
                                            <Form.Control as="textarea" rows="3" onChange={this.UpdateDescriptionchangehandler} name="Description" value={this.state.viewdDescription}   readOnly />
                                        </Form.Group>
                                    </Col>
                                    <Col sm={6}>
                                        <Form.Group controlId="DueDate">
                                            <Form.Label> DueDate</Form.Label>
                                            <Form.Control type="date" onChange={this.UpdateDuedatechangehandler} name="Duedate" value={this.state.viewDuedate}  readOnly  />
                                        </Form.Group>
                                    </Col>
                                    <Col sm={6}>
                                        <Form.Group controlId="Priority">
                                            <Form.Label> Priority</Form.Label>
                                             <Form.Control as="select" onChange={this.UpdatePrioritychangehandler} name="Priority" value={this.state.viewPriority}  readOnly  >
                                                <option selected>Select Value</option>
                                                <option value="none">None</option>
                                                <option value="low">Low</option>
                                                <option value="medium">Medium</option>
                                ...
                                <Row>
                                    <Col sm={12}>
                                         <div className="DeleteModal">
                                             ...
                                             <Button variant="secondary" type="submit" className="themesflat-button blue mr-2" onClick={this.modalClosehandle}> Cancel</Button>
                                             <Button variant="success" type="submit" variant="danger" id={this.state.Deletetaskid} onClick={this.DeleteTaskhandler}>Delete</Button>
                                        </div>
                                    </Col>
                                </Row>
                        </Modal.Body>
